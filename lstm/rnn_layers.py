...
def rnn_step_backward(dnext_h, cache):
    ...
    dx = (dtanh * dnext_h) @ (Wx.T)
    dprev_h = (dtanh * dnext_h) @ (Wh.T)
    dWx = x.T @ (dtanh * dnext_h)
    dWh = prev_h.T @ (dtanh * dnext_h)
...
def rnn_backward(dh, cache):
    ...
    db = np.zeros(H)
    for i in range(T):
      cur_cache = cache[T - i - 1]
      cdx, cdh, cdWx, cdWh, cdb = rnn_step_backward(dh[:, T - i - 1, :] + cdh, cur_cache)
      dx[:, T - i - 1, :] = cdx
      dh0 = cdh
      dWx += cdWx      
      db += cdb
...
def lstm_backward(dh, cache):
    ...
    for i in range(T):
      cur_cache = cache[T - i - 1]
      cdx, cdh, cdc, cdWx, cdWh, cdb = lstm_step_backward(dh[:, T - i - 1, :] + cdh, cdc, cur_cache)
      dx[:, T - i - 1, :] = cdx
      dh0 = cdh
      dWx += cdWx      
      db += cdb
